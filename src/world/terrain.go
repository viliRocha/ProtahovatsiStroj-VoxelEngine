...
func shapeTerrain(worley *WorleyNoise, biomeSel *BiomeSelector, position rl.Vector3, x, z int, p1, p2, p3 *perlin.Perlin) (int, BiomeProperties) {
	gx := int(position.X) + x
	gz := int(position.Z) + z
			...
			for y := 0; y < pkg.WorldHeight; y++ {
				...
				if isSolid {
					...
					if y == height && y > waterLevel {
						chunk.Voxels[x][y][z] = pkg.VoxelData{Type: biome.SurfaceBlock}
					} else if y <= height-5 {
